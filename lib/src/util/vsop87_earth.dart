...
const double julianDayJan1_2000 = 2451545.0;

double sinDeg(double angle) => sin(radians(angle));

double cosDeg(double angle) => cos(radians(angle));

double tanDeg(double angle) => tan(radians(angle));

double acosDeg(double value) => degrees(acos(value));

double asinDeg(double value) => degrees(asin(value));

const double julianDaysPerCentury = 36525.0;

double julianDayOfDate(DateTime date) {
  int year = date.year;
...
double julianDayOfInstant(DateTime instant) =>
    julianDayOfDate(instant.toUtc()) + fractionalDayOfInstant(instant);

double fractionalDayOfInstant(DateTime instant) {
...
double estimateDeltaT(double julianDay) {
  final double year = 2000.0 + (julianDay - julianDayJan1_2000) / 365.25;
  double t;
  if (year >= 2005 && year < 2050) {
    ...
    final double u = (year - 1820) / 100.0;
    return -20 + 32 * u * u;
  }
}

double sumSeries(List<List<List<double>>> series, double tau) {
  double total = 0.0;
  double tauPow = 1.0;
  for (final List<List<double>> terms in series) {
    double sum = 0.0;
...
const List<List<List<double>>> earthL = [
  [
...
const List<List<List<double>>> earthB = [
  [
...
const List<List<List<double>>> earthR = [
  [
