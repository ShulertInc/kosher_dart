...
///   constructor [GeoLocation.withElevation], and correct storage of all fields.
///
...
void main() {
  ...
  group('GeoLocation - withElevation constructor', () {
    test('stores name, lat, lon, elevation correctly', () {
      final g = GeoLocation.withElevation('Lakewood, NJ', 40.0828, -74.2094, 20.0, tz.UTC);
  ...
  group('GeoLocation - pole and equator latitudes', () {
    ...
    test('latitude > 90 throws ArgumentError', () {
      final g = GeoLocation();
      expect(() => g.setLatitude(90.1), throwsArgumentError);
    });
  ...
  group('GeoLocation - meridian longitudes', () {
    ...
    test('longitude > 180 throws ArgumentError', () {
      final g = GeoLocation();
      expect(() => g.setLongitude(180.1), throwsArgumentError);
    });
  ...
  group('GeoLocation - antimeridian adjustment', () {
    test('typical location returns 0 adjustment', () {
      ...
      final g = GeoLocation.withZoneId('New York', 40.7127, -74.0059, tz.UTC);
      expect(g.getAntimeridianAdjustment(DateTime.utc(2024, 1, 1)), equals(0));
    });

    test('Jerusalem returns 0 adjustment', () {
      final g = GeoLocation.withZoneId('Jerusalem', 31.7964, 35.2454, tz.UTC);
      expect(g.getAntimeridianAdjustment(DateTime.utc(2024, 1, 1)), equals(0));
    });
  });
  ...
  group('GeoLocation - geodesic distance', () {
    test('distance from a point to itself is 0', () {
      final nyc = GeoLocation.withZoneId('NYC', 40.7127, -74.0059, tz.UTC);
      final nyc2 = GeoLocation.withZoneId('NYC', 40.7127, -74.0059, tz.UTC);
      expect(nyc.getGeodesicDistance(nyc2), closeTo(0, 1));
    });

    test('distance NYC to Jerusalem is approximately 9130 km', () {
      final nyc = GeoLocation.withZoneId('NYC', 40.7127, -74.0059, tz.UTC);
      final jerusalem = GeoLocation.withZoneId('Jerusalem', 31.7964, 35.2454, tz.UTC);
    ...
    test('distance is symmetric', () {
      final a = GeoLocation.withZoneId('A', 51.5074, -0.1278, tz.UTC); // London
      final b = GeoLocation.withZoneId('B', 48.8566, 2.3522, tz.UTC); // Paris
    ...
    test('equatorial quarter-circumference: distance ~10 007 km', () {
      ...
      final a = GeoLocation.withZoneId('A', 0.0, 0.0, tz.UTC);
      final b = GeoLocation.withZoneId('B', 0.0, 90.0, tz.UTC);
    ...
    test('antipodal equatorial points return NaN (Vincenty known limitation)', () {
      ...
      final a = GeoLocation.withZoneId('A', 0.0, 0.0, tz.UTC);
      final b = GeoLocation.withZoneId('B', 0.0, 180.0, tz.UTC);
      final dist = a.getGeodesicDistance(b);
  ...
  group('GeoLocation - rhumb line distance', () {
    test('rhumb distance from a point to itself is 0', () {
      final g = GeoLocation.withZoneId('Tel Aviv', 32.0853, 34.7818, tz.UTC);
      final g2 = GeoLocation.withZoneId('Tel Aviv', 32.0853, 34.7818, tz.UTC);
  ...
  group('GeoLocation - clone', () {
    test('clone produces equal values', () {
      final original = GeoLocation.withElevation('Jerusalem', 31.7964, 35.2454, 834.0, tz.UTC);
  ...
  group('GeoLocation - local mean time offset', () {
    test('prime meridian UTC has offset ~0', () {
      ...
      final g = GeoLocation.withZoneId('Greenwich', 51.4772, 0.0, tz.UTC);
      // LMT offset = lon * 4 * 60000 - tzOffsetMs; at UTC, tzOffset=0
      expect(g.getLocalMeanTimeOffset(DateTime.utc(2024, 1, 1)), closeTo(0, 1000));
    });
  });
}
